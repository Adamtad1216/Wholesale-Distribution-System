...
class PaymentService {
  ...
  async getProvidersAndMethods() {
    ...
    if (!providers || providers.length === 0) {
      // Auto-seed default Payment Providers & Methods into database
      await prisma.paymentProvider.createMany({
        data: [
          { name: 'Commercial Bank of Ethiopia (CBE Wire)', code: 'cbe_wire', type: 'ONLINE', isActive: true },
          { name: 'Telebirr Digital Gateway', code: 'telebirr', type: 'ONLINE', isActive: true },
          { name: 'Chapa Gateway', code: 'chapa', type: 'ONLINE', isActive: true },
          { name: 'Company Petty Cash', code: 'petty_cash', type: 'MANUAL', isActive: true },
          { name: 'Company Commercial Cheque', code: 'company_check', type: 'MANUAL', isActive: true },
          { name: 'Bank Deposit Slip', code: 'deposit_slip', type: 'MANUAL', isActive: true }
        ],
        skipDuplicates: true
      });
      ...
      // Add default method options for seeded providers
      for (const p of providers) {
        if (p.methods.length === 0) {
          if (p.code === 'cbe_wire') {
            await prisma.paymentMethodOption.createMany({
              data: [
                { providerId: p.id, name: 'CBE Direct Wire Transfer', code: 'CBE_DIRECT', requiresProof: true, isActive: true },
                { providerId: p.id, name: 'CBE Birr App Transfer', code: 'CBE_BIRR', requiresProof: false, isActive: true }
              ]
            });
          } else if (p.code === 'telebirr') {
            await prisma.paymentMethodOption.createMany({
              data: [
                { providerId: p.id, name: 'Telebirr Merchant Pay', code: 'TELEBIRR_PAY', requiresProof: false, isActive: true }
              ]
            });
          } else if (p.code === 'petty_cash') {
            await prisma.paymentMethodOption.createMany({
              data: [
                { providerId: p.id, name: 'Petty Cash Disbursement Voucher', code: 'CASH_VOUCHER', requiresProof: true, isActive: true }
              ]
            });
          } else if (p.code === 'company_check') {
            await prisma.paymentMethodOption.createMany({
              data: [
                { providerId: p.id, name: 'Issued Bank Cheque', code: 'CHEQUE_PAYMENT', requiresProof: true, isActive: true }
              ]
            });
          } else if (p.code === 'deposit_slip') {
            await prisma.paymentMethodOption.createMany({
              data: [
                { providerId: p.id, name: 'Bank Teller Receipt Slip', code: 'TELLER_SLIP', requiresProof: true, isActive: true }
              ]
            });
          }
        }
