  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
        ...
        {/* About Card */}
        <Card className="lg:col-span-2 space-y-5 border border-border ">
        ...
        {customer && (
          <Card className="lg:col-span-2 space-y-4 border border-border  mt-4">
            <div className="flex items-center gap-2.5">
              <div className="p-1.5 rounded-lg border flex items-center justify-center bg-[var(--icon-box-bg)] text-[var(--icon-box-text)] border-[var(--icon-box-border)]">
                <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
              ...
              {customer.organization.contacts && customer.organization.contacts.length > 0 && (
                <div className="sm:col-span-2">
                  ...
                  <Card noPadding className="flex items-center gap-3 p-3">
                    <div className="w-10 h-10 rounded-full bg-blue-600  font-bold flex items-center justify-center text-foreground">
        ...
        <div className="space-y-4">
          <div className="flex items-center gap-2.5 px-1 pb-1 border-b border-border ">
            <div className="p-1.5 rounded-lg border flex items-center justify-center bg-[var(--icon-box-bg)] text-[var(--icon-box-text)] border-[var(--icon-box-border)]">
              <svg className="w-5 h-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
            ...
            {[
              {
                ...
                value: (
                  <span className="inline-flex items-center gap-1.5 px-2.5 py-0.5 rounded-full text-xs font-bold border bg-[var(--icon-box-bg)] text-[var(--icon-box-text)] border-[var(--icon-box-border)]">
              ...
              {
                label: 'Last Active',
                value: <span className="text-xs font-semibold text-emerald-600 ">{user?.lastLoginAt ? new Date(user.lastLoginAt).toLocaleString('en-US', { month: 'short', day: 'numeric', hour: '2-digit', minute: '2-digit' }) : 'Active now'}</span>,
                icon: 'M12 8v4l3 3m6-3a9 9 0 11-18 0 9 9 0 0118 0z',
                color: 'ui-icon-box-teal',
              },
              {
                label: 'Uploaded Documents',
                value: (
                  <span className="text-xs font-bold px-2 py-0.5 rounded-md bg-sky-100 text-sky-700   border border-sky-200 ">
                    {user?._count?.documents ?? user?.documents?.length ?? 0} Docs
                  </span>
                ),
                icon: 'M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z',
                color: 'ui-icon-box-sky',
              },
            ].map(({ label, value, icon, color }) => (
              <Card key={label} noPadding className="p-3 flex items-center justify-between border border-border  hoverEffect">
                <div className="flex items-center gap-3 min-w-0">
                  <div className="w-9 h-9 rounded-lg border flex items-center justify-center shrink-0 bg-[var(--icon-box-bg)] text-[var(--icon-box-text)] border-[var(--icon-box-border)]">
