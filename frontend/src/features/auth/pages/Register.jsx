...
const registerValidationSchema = z.discriminatedUnion('customerType', [
  ...
  z.object({
    ...
    email: z.string().email('Invalid email address').or(z.literal('')),
    address: z.string().max(255).optional(),
    contactFirstName: z.string().min(1, 'Primary contact first name is required'),
    contactLastName: z.string().min(1, 'Primary contact last name is required'),
    contactEmail: z.string().email('Invalid contact email').or(z.literal('')),
    contactPhone: z.string().or(z.literal('')),
  }),
]);
...
export default function Register() {
  ...
  const mutation = useMutation({
    ...
    onError: (err) => {
      const errMsg = err.message || 'Registration failed. Username or email might be taken.';
      toast.error(errMsg);
    },
  });
        ...
        <form onSubmit={handleSubmit(onSubmit)} className="space-y-6">
          ...
          {customerType === 'PERSON' ? (
            <div className="space-y-4">
              <h3 className="text-sm font-semibold text-violet-400">Personal Details</h3>
            ...
            <div className="space-y-4">
              <h3 className="text-sm font-semibold text-violet-400">Organization Details</h3>
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div className="md:col-span-2">
                  <label className="block text-xs text-slate-400 mb-1">Organization / Company Name</label>
                  <input
                    type="text"
                    {...register('name')}
                    className="w-full px-4 py-2 rounded-xl bg-slate-900/60 border border-slate-800 focus:border-violet-500/50 text-sm outline-none"
                ...
                <div>
                  <label className="block text-xs text-slate-400 mb-1">Tax Number (TIN)</label>
                  <input
                    type="text"
                    {...register('taxNumber')}
                    className="w-full px-4 py-2 rounded-xl bg-slate-900/60 border border-slate-800 focus:border-violet-500/50 text-sm outline-none"
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  <label className="block text-xs text-slate-400 mb-1">Company Phone</label>
                  <input
                    type="text"
                    {...register('phone')}
                    className="w-full px-4 py-2 rounded-xl bg-slate-900/60 border border-slate-800 focus:border-violet-500/50 text-sm outline-none"
                ...
                <div>
                  <label className="block text-xs text-slate-400 mb-1">Company Address</label>
                  <input
                    type="text"
                    {...register('address')}
                    className="w-full px-4 py-2 rounded-xl bg-slate-900/60 border border-slate-800 focus:border-violet-500/50 text-sm outline-none"
              ...
              <h3 className="text-sm font-semibold text-violet-400">Primary Contact Person</h3>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-xs text-slate-400 mb-1">First Name</label>
                  <input
                  ...
                  {errors.contactFirstName && (
                    <p className="text-xs text-rose-500 mt-1">{errors.contactFirstName.message}</p>
                  )}
                </div>
                <div>
                  <label className="block text-xs text-slate-400 mb-1">Last Name</label>
                  ...
                  {errors.contactLastName && (
                    <p className="text-xs text-rose-500 mt-1">{errors.contactLastName.message}</p>
                  )}
                </div>
                <div>
          ...
          {/* Account Credentials (rendered last) */}
          <hr className="border-slate-800/40 my-4" />
          <h3 className="text-sm font-semibold text-violet-400">Account Credentials</h3>
