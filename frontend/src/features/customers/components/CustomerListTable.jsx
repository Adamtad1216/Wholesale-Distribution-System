...
export default function CustomerListTable({
  ...
  if (customers.length === 0) {
    return (
      <Card className="p-12 text-center text-muted-foreground text-sm rounded-2xl border border-border bg-card shadow-sm space-y-3">
      ...
      <Table containerClassName="rounded-2xl border border-border bg-card shadow-sm">
        ...
        <TableBody>
          {customers.map((c) => {
            ...
            const isOrg = c.customerType === 'ORGANIZATION';
            ...
            return (
              <TableRow key={c.id}>
                ...
                {/* Payment Terms */}
                <TableCell>
                  <span className="px-2.5 py-1 rounded-full text-xs font-semibold badge-slate inline-block">
                    {c.paymentTerms?.name || 'COD (0 Days)'}
                  </span>
                </TableCell>
                ...
                <TableCell>
                  <span className={`px-2.5 py-1 rounded-full text-xs font-semibold inline-flex items-center gap-1.5 ${
                    ...
                    <span className={`w-1.5 h-1.5 rounded-full ${
                      c.status === 'ACTIVE' ? 'bg-emerald-400' : c.status === 'SUSPENDED' ? 'bg-rose-400' : 'bg-amber-400'
                    }`}></span>
                    {c.status || 'ACTIVE'}
                  </span>
                </TableCell>
                ...
                <TableCell className="text-left">
                  <div className="flex items-center justify-start gap-2">
                    ...
                    <button
                      ...
                      className="px-2.5 py-1.5 bg-secondary hover:bg-secondary/80 border border-border rounded-xl text-foreground font-semibold text-xs inline-flex items-center gap-1.5 transition shadow-sm cursor-pointer"
                    >
                    ...
                    {canUpdate && (
                      <button
                        ...
                        className="px-2.5 py-1.5 bg-indigo-500/10 hover:bg-indigo-500/20 border border-indigo-500/30 rounded-xl text-indigo-400 hover:text-indigo-300 font-semibold text-xs inline-flex items-center gap-1.5 transition shadow-sm cursor-pointer"
                      >
                    ...
                    {canDelete && (
                      <button
                        onClick={() => handleDeleteCustomer(c.id, displayName)}
                        title="Delete Customer"
                        className="px-2.5 py-1.5 bg-rose-500/10 hover:bg-rose-500/20 border border-rose-500/30 rounded-xl text-rose-400 hover:text-rose-300 font-semibold text-xs inline-flex items-center gap-1.5 transition shadow-sm cursor-pointer"
                      >
                        <svg className="w-3.5 h-3.5" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth="2">
      ...
      <div className="flex flex-col sm:flex-row items-center justify-between gap-4 pt-2">
        ...
        <div className="flex items-center gap-2">
          <select
            ...
            className="px-2.5 py-1.5 bg-background border border-border rounded-xl text-foreground text-xs focus:outline-none cursor-pointer"
          >
